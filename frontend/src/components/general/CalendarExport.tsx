...
export default function CalendarExport({
  ...
  const createCalendarEvents = (): CalendarEvent[] => {
    ...
    // new Date("2026-02-12") is parsed as UTC and renders as Feb 11 in US timezones.
    ...
    // Create events for selected lectures (recurring weekly)
    selectedLectures.forEach(lecture => {
      const startDate = parseLocalDate(lecture.start_date)
      const endDate = parseLocalDate(lecture.end_date)
    ...
    // Create events for selected assignments
    selectedAssignments.forEach(assignment => {
      const dueDate = parseLocalDate(assignment.date)
